...
<?php 
$status='Pending';
$query=mysqli_query($con,"SELECT * FROM orders WHERE orderStatus='$status' || orderStatus is null order by id DESC");
$cnt=1;
...
?>										
							...
							<td><?php echo htmlentities($row['DatePosted']);?></td>	
										<td> 
											  ...
											  <a href="pending-orders.php?id=<?php echo $row['id']?>&del=delete" onClick="return confirm('Are you sure you want to delete?')"><i class="icon-remove-sign"></i></a>
										</td>
											</tr>
										<?php $cnt=$cnt+1; } ?>										
								</table>
